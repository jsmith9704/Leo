...
(function () {
  ...
  const WORKFLOWS = [{
    ...
    patient: "M. Okonkwo",
    patientMeta: "67F · MRN 04‑881‑2207",
    dept: "Cardiology",
    origin: "Primary Care — Ridgeline Clinic",
    stage: "Consult complete · loop open",
    risk: 82,
    trend: +14,
    dominant: "intent",
    scores: {
    ...
    owners: ["Referral Coordination", "Cardiology Intake"],
    lastEvent: "2d ago",
    status: "At risk",
    breaks: 5,
    primary: true
    ...
    patient: "R. Delacroix",
    patientMeta: "74M · MRN 03‑552‑9910",
    dept: "Med‑Surg 4E",
    origin: "Inpatient discharge",
    stage: "Discharged · no follow-up booked",
    risk: 91,
    trend: +6,
    dominant: "ownership",
    scores: {
    ...
    patient: "S. Whitfield",
    patientMeta: "59F · MRN 05‑113‑7741",
    dept: "Laboratory",
    origin: "Chem panel — critical K+",
    stage: "Result released · ack pending",
    risk: 76,
    trend: -3,
    ...
    patient: "T. Abara",
    patientMeta: "52M · MRN 02‑447‑1180",
    dept: "Endocrinology",
    origin: "Consult note authored",
    stage: "Note signed · not routed",
    risk: 71,
    trend: +2,
    ...
    patient: "L. Nakamura",
    patientMeta: "48F · MRN 04‑220‑6651",
    dept: "Pharmacy",
    origin: "e-Prescribe — PA required",
    stage: "Auth approved · fill pending",
    risk: 54,
    trend: -8,
    dominant: "timing",
    scores: {
    ...
    patient: "D. Castellano",
    patientMeta: "81M · MRN 01‑903‑4412",
    dept: "Emergency",
    origin: "Admit decision",
    stage: "Boarding · handoff in progress",
    risk: 48,
    trend: +1,
    dominant: "action",
    scores: {
    ...
    patient: "J. Pereira",
    patientMeta: "39F · MRN 05‑667‑2093",
    dept: "Radiology",
    origin: "CT abdomen ordered",
    stage: "Read finalized · acknowledged",
    risk: 28,
    trend: -11,
    dominant: "verification",
    scores: {
  ...
  const TIMELINE = [{
    seq: 1,
    t: "May 04 · 09:12",
    phase: "Decision to refer",
    actor: "Dr. A. Alvarez",
    role: "PCP · Ridgeline Clinic",
    system: "EHR · Encounter",
    status: "ok",
    ...
    seq: 2,
    t: "May 04 · 09:26",
    phase: "Referral order placed",
    actor: "Dr. A. Alvarez",
    role: "PCP",
    system: "EHR · Order entry",
    status: "degraded",
    ...
    seq: 3,
    t: "May 05 · 14:03",
    phase: "Routed to scheduling",
    actor: "Referral Coordination",
    role: "Queue transfer",
    system: "Referral mgmt",
    status: "ok",
    ...
    seq: 4,
    t: "May 06 – May 12",
    phase: "Insurance authorization",
    actor: "Authorization Team",
    role: "Payer auth",
    system: "Auth portal",
    status: "degraded",
    ...
    seq: 5,
    t: "May 13 · 10:41",
    phase: "Patient outreach",
    actor: "Cardiology Intake",
    role: "Scheduler",
    system: "Phone · Telephony",
    status: "degraded",
    ...
    seq: 8,
    t: "May 28 · 16:44",
    phase: "Consult note authored",
    actor: "Dr. M. Boone",
    role: "Cardiology",
    system: "EHR · Documentation",
    status: "ok",
  ...
  const EVIDENCE = {
    ...
    "EV-104": {
      id: "EV-104",
      type: "Authorization log",
      source: "Payer auth portal",
      actor: "Authorization Team",
      t: "May 06 – May 12",
      dim: "timing",
  ...
  const WORKSPACE = {
    "F-01": {
      ...
      verificationPath: [{
        step: 1,
        action: "Review original encounter note (EV-101) — confirm anticoagulation language is present and unambiguous."
      }, {
        step: 2,
        action: "Check referral order structured fields (EV-102) — confirm 'anticoagulation' is absent from Reason and all free-text fields."
      }, {
        step: 3,
        action: "Review consult note (EV-107) in full — search assessment, plan, HPI, and any addenda for any anticoagulation mention."
      }, {
        step: 4,
        action: "Check for any in-basket messages, co-signature requests, or phone notes between Dr. Alvarez and Dr. Boone before or after May 28."
      }, {
        step: 5,
        action: "Review patient's current medication list — confirm no anticoagulant was started, indicating the decision remains unresolved."
      }],
    ...
    "F-02": {
      ...
      verificationPath: [{
        step: 1,
        action: "Confirm referral order (EV-102) priority field reads URGENT ≤7d — check for any amendment or correction after placement."
      }, {
        step: 2,
        action: "Confirm scheduling record (EV-106): appointment class ROUTINE, booked date 05/28, lead time 24 days."
      }, {
        step: 3,
        action: "Review scheduling system UI configuration for May 13 — confirm urgency indicator was not visible to the scheduler at booking time."
      }, {
        step: 4,
        action: "Search for any supervisor approval, override note, or patient-preference documentation that could justify ROUTINE booking."
      }, {
        step: 5,
        action: "Review telephony log for May 13 outreach calls — confirm whether urgency was communicated to the patient verbally."
      }],
    ...
    "F-03": {
      ...
      verificationPath: [{
        step: 1,
        action: "Check referral management system: confirm WF-2287 current state and next_owner field values in real time."
      }, {
        step: 2,
        action: "Check Dr. Alvarez's in-basket — confirm no consult result message, co-signature request, or result notification is pending or unread."
      }, {
        step: 3,
        action: "Review Cardiology Intake task queue — confirm no loop-closure or result-routing task exists for WF-2287."
      }, {
        step: 4,
        action: "Review EHR chart activity since May 28 — look for any PCP-authored note, addendum, or medication change referencing the consult."
      }],
    ...
    "F-04": {
      ...
      verificationPath: [{
        step: 1,
        action: "Confirm auth log (EV-104): PA opened 05/06, approved 05/12, elapsed 6d 6h — check for any interim communication or expedite request."
      }, {
        step: 2,
        action: "Review referral management system: confirm the 7-day urgent target was visible to the auth team or system during processing."
      }, {
        step: 3,
        action: "Check auth portal configuration for this referral type — is there an urgent-window escalation rule? What threshold triggers it?"
      }, {
        step: 4,
        action: "Review scheduling system for any booking attempt before May 12 — confirm no pre-auth scheduling was attempted or was even possible."
      }],
      affectedDims: ["timing"]
    },
    "F-05": {
      ...
      verificationPath: [{
        step: 1,
        action: "Check Dr. Alvarez's in-basket — is the consult note routed for review, and if so, has it been opened?"
      }, {
        step: 2,
        action: "Review Cardiology Documentation routing rules — does a signed consult note auto-route to the referring provider?"
      }, {
        step: 3,
        action: "Check PCP encounter activity since May 28 — any note, medication change, or patient communication referencing the cardiology consult?"
      }, {
        step: 4,
        action: "Review referral system closure protocol — is PCP acknowledgment required before status transitions from 'consult_complete' to 'closed'?"
      }],
